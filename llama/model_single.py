...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # It also breaks compatibility with the standard generate function
        # (which feeds inputs one token at the time, and expects the model to
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor):
        ...
        idx = torch.arange(0, seqlen, device=h.device)
        
        return self.forward_raw(idx, h, integration_technique='riemann')

    @torch.inference_mode()
    def forward_raw(self, idx: torch.Tensor, h: torch.Tensor, integration_technique='riemann', integration_start=0):
        seqlen = h.shape[1]

        freqs_cis = compute_freqs_cis(idx, self.params.dim // self.params.n_heads)
        #freqs_cis = compute_freqs_cis(torch.arange(seqlen, device=h.device), self.params.dim // self.params.n_heads)
